...
export const refundInput = z
  .object({
    transactionId: z.string().regex(/^TXN_[A-Z0-9_]+$/),
    amountMinor: amount,
  ...
  try {
    return await mongoose.connection.transaction(async (session) => {
      const payment = await Payment.findOneAndUpdate(
            ...
            $gte: [
              {
                $subtract: [
                  "$amountMinor",
                  { $add: ["$refundedMinor", "$reservedRefundMinor"] },
                ],
              },
              input.amountMinor,
            ],
        ...
        { $inc: { reservedRefundMinor: input.amountMinor } },
        { session, new: true },
      );
      if (!payment)
        throw new AppError(
          409,
          "REFUND_NOT_ELIGIBLE",
          "Payment is not eligible or the amount exceeds the refundable balance",
        );
        ...
        [
          {
            ...
            source: "api",
            metadata: { refundId, amountMinor: input.amountMinor },
          },
        ],
        { session, ordered: true },
      );
        ...
        [
          {
            ...
            organization: actor.organization,
            kind: "REFUND",
            payload: { refundId, outcome: "SUCCESS" },
            createdBy: actor.userId,
          },
        ],
        { session, ordered: true },
...
export const webhookInput = z
  .object({
    ...
    organization: z.string().min(4).max(100),
    type: z.enum([
      "payment.success",
      "payment.failed",
      "refund.success",
  ...
  .strict();
export function signWebhook(raw: Buffer, timestamp: string) {
  return createHmac("sha256", config.WEBHOOK_SECRET)
    .update(timestamp + ".")
    .update(raw)
    .digest("hex");
}
export function verifyWebhook(
  ...
  if (
    ...
    throw new AppError(401, "WEBHOOK_EXPIRED", "Webhook timestamp is invalid");
...
export async function applyWebhook(input: z.infer<typeof webhookInput>) {
  ...
  try {
    return await mongoose.connection.transaction(async (session) => {
      ...
      if (input.type.startsWith("payment.")) {
        ...
        if (!["CREATED", "PENDING", "AUTHORIZED"].includes(payment.status))
          return { ignored: true };
        payment.status =
          input.type === "payment.success" ? "SUCCESS" : "FAILED";
        if (payment.status === "SUCCESS") {
          payment.ledgerDebitMinor = payment.amountMinor;
          payment.merchantCreditMinor = payment.amountMinor;
        }
        ...
        if (["SUCCESS", "FAILED"].includes(refund.status))
          return { ignored: true };
        const success = input.type === "refund.success";
        ...
        const payment = await Payment.findOneAndUpdate(
          {
            organization: input.organization,
            transactionId,
            reservedRefundMinor: { $gte: refund.amountMinor },
          },
          {
            $inc: {
              reservedRefundMinor: -refund.amountMinor,
              refundedMinor: success ? refund.amountMinor : 0,
              refundLedgerMinor: success ? refund.amountMinor : 0,
            },
          },
          { new: true, session },
        );
          ...
          [
            {
              ...
              organization: input.organization,
              transactionId,
              eventType: success ? "REFUND_SUCCESS" : "REFUND_FAILED",
              source: "gateway",
        ...
        if (settlement.status !== "COMPLETED")
          throw new AppError(
            ...
            "Settlement must finish processing before confirmation",
          );
      }
      await Audit.create(
        [
          {
            ...
            action: input.type.toUpperCase().replace(".", "_"),
            entityType: "transaction",
        ...
        [
          {
            ...
            payload: {
              transactionId:
                input.type === "settlement.completed"
                  ? undefined
                  : transactionId,
            },
          },
    ...
    const winner = await Webhook.findOne({ eventId: input.eventId });
    if (winner?.fingerprint === hash) return { duplicate: true };
    throw error;
  }
}
